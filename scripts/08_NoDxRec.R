...
slopebmregdf_nodx <- slopebmregdf |>
  mutate(bmdf_nodx = map(bmdf, inner_join, sel_ids, by = "eid"))
...
slopebmregdf_nodx <- slopebmregdf_nodx |>
  mutate(
    mod1_nodx = map(
      bmdf_nodx,
      ~bam(
