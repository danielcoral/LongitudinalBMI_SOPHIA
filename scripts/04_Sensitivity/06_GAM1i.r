...
lmmtab1i <- readRDS(
  file.path(projfld, "data", "UKB", "processed_data", "lmmtab1i.rds")
)
...
slopebmregdf_1i <- bm1i |> inner_join(cv1i, by = "eid")
...
slopebmregdf_1i <- slopebmregdf_1i |>
  select(
    eid, sex, age, bmi,
    all_of(covars),
    all_of(bmrks)
...
slopebmregdf_1i <- slopebmregdf_1i |>
  nest(Dat = -sex) |>
  inner_join(lmmtab1i, by = "sex") |>
  transmute(
    sex,
    Dat = map2(indiv_est, Dat, inner_join, by = "eid"),
    Dat = map(Dat, nest, bmdf_1i = -trait)
  ) |>
  unnest(Dat)

print(slopebmregdf_1i)
...
pdiffdf <- slopebmregdf |>
  inner_join(slopebmregdf_1i, by = c("sex", "trait")) |>
  transmute(
    sex, trait,
    modpred_all = map(
      mod1,
...
pdiffdf |>
  ...
  mutate(
    ...
    Subset = factor(
      Subset,
      levels = c("all", "1i"),
      labels = c("Recruitment", "Second visit")
    )
  ) |>
  nest(plotdf = -grp2) |>
  mutate(
    plotdf = map(
      ...
      function(x) {
        x |>
          ...
          geom_hline(aes(yintercept = thresh), lty = "11", linewidth = .2) +
          geom_linerange(
            aes(ymin = conf.low, ymax = conf.high, group = Subset),
            position = position_dodge(width = 1.25),
            linewidth = .3
          ) +
          geom_point(
            aes(group = Subset, color = Subset),
            position = position_dodge(width = 1.25),
            size = .5
          ) +
          ggh4x::facet_nested(
