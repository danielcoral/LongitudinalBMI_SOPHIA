...
lmmtabrec_s <- readRDS(
  file.path(projfld, "data", "UKB", "processed_data", "lmmtabrec_s.rds")
)
...
slopebmregdf_s <- lmmtabrec_s |>
  select(sex, indiv_est) |>
  inner_join(slopebmregdf, by = "sex") |>
  mutate(
    bmdf_s = map2(
      bmdf, indiv_est,
      ~.x |>
...
pdiffdf |>
  ...
  mutate(
    ...
    Subset = factor(
      Subset,
      levels = c("all", "s"),
      labels = c("All", "Stricter criteria")
    )
  ) |>
  nest(plotdf = -grp2) |>
  mutate(
